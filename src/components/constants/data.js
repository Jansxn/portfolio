import nontouch from '../images/non-touch-writing-pad.jpg'
import website from '../images/website.png'
import linefollow from '../images/linefollow.png'

export const techstacks = [
    ['Python', 'JavaScript' ,'C', 'C++', 'Java', 'Shell (Bash)'],
    ['HTML', 'CSS', 'JavaScript', 'React.js', 'Tailwind.css']
]

export const exp = [
    {
        id:0,
        job:'Robotics and Circuits (student project)',
        extra:'currently empolyed',
        desc:['Worked in the coding domain for 2 years', 
            'Learnt arduino and machine learning in python',
            'Built projects such as Line following bot amongst others' ,
            'Won 2nd place in MITs Techtatva'],
        link:'https://roboticsandcircuits.com/',
        time:'November \'22 - Present'

    },
    {
        id:1,
        job:'ACM Manipal (student club)',
        extra:'currently empolyed',
        desc:['Worked in the competitive-coding, web-dev domains'],
        link:'https://manipal.acm.org/',
        time:'April \'22 - Present'
    }
]

export const projects = [
    {
        id: 0,
        name: 'Line-Following Bot',
        desc: ['Developed the code for a bot that follows a black line on a white surface using sensors and motors',
            'Used Arduino to code the bot',
            'Bot was part of my taskphase project for Robotics and Circuits',
            'P.S. no github link'],
        link: '',
        img: linefollow
    },
    {
        id: 1,
        name: 'Non Touch Writing Pad',
        desc: ['Designed a software that allows users to control a system using purely hand gestures',
            'Used OpenCV to detect hand gestures and machine learning to recognize characters drawn ',
            'Worked on front-end and back-end of the project along with 2 other team members from Robotics and Circuits',
            'Won 2nd place in Vedant 12.0 at MITs Techtatva'],
        link: 'https://github.com/RNCManipal/Non-Touch_Writing_Pad',
        img : nontouch
    },
    {
        id: 2,
        name: 'Portfolio Website',
        desc: ['Developed this website using React.js and Tailwind.css',
            'Used React.js to make the website update the style and content dynamically when new content is added to a json file',
            'Used Tailwind.css as well as vanilla css to make the webite responsive across all devices'], 
        link: 'https://github.com/Jansxn/portfolio_app' ,
        img: website
    }
]